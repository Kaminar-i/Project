...
pub trait IStudentRegistry<TContractState> {
    fn Register_stud(ref self: TContractState, student_addr: ContractAddress, _name: felt252, age: u8);
    fn Get_stud(self: @TContractState, student_addr: ContractAddress) -> Student;
    fn Update_stud(ref self: TContractState, student_addr: ContractAddress, new_student_addr: ContractAddress,new_name: felt252, new_age: u8);
...
mod StudentRegistry {
    ...
    impl StudentRegistry of super::IStudentRegistry<ContractState> {
        fn Register_stud(ref self: ContractState, student_addr: ContractAddress, _name: felt252, age: u8){
            let student = Student {
                student_addr,
                _name,
                age,
        ...
        fn Update_stud(ref self: ContractState, student_addr: ContractAddress, new_student_addr: ContractAddress, new_name: felt252, new_age: u8){
            // let zero_address: ContractAddress = 0x000.try_into().unwrap();
            ...
            let updated_student = Student {
                student_addr: new_student_addr,
                _name: new_name,
                age: new_age,
            };

            self.students.write(new_student_addr, updated_student);
        ...
        fn Delete_stud(ref self: ContractState, student_addr: ContractAddress){
            ...
            self.emit(Event::DeleteStudent(DeleteStudent { user: student_addr, name: student_name}));
